...
contract LenderManagement {
    ...
    function createLendingContract (uint split, string memory nodeType, uint fee, string memory contractText) public payable {
    ...
    function addContractMessage(address contractAddress, string memory contractMessage, string memory messageSide) public {
...
contract NodeLender {
    ...
    constructor(uint split, string memory contractType, uint fee) public payable {
        lender = tx.origin;
...
contract LendingController {
    
    LenderManagement public lenderManagementContract;
    ...
    constructor(address lendingContractAddress) public {
        lenderManagementContract = LenderManagement(lendingContractAddress);
    }
    
    function getLendingContractData() public view returns (lendingContract[] memory) {
        lendingContract[] memory ret = new lendingContract[](lenderManagementContract.lendingContractCount());
        for (uint i = 0; i < lenderManagementContract.lendingContractCount(); i++) {
            address lendingContractAddress = lenderManagementContract.lendingContractCountMapping(i);
            var (nodeType, index, lenderIndex, borrowerIndex, lenderAddress, borrowerAddress, contractAddress, originationFee, available, lenderSplit, text) = lenderManagementContract.lendingContractMapping(lendingContractAddress);
            lendingContract memory newContract = lendingContract({nodeType:nodeType, index:index, lenderIndex:lenderIndex, borrowerIndex:borrowerIndex, lenderAddress:lenderAddress, borrowerAddress:borrowerAddress, lendingContractAddress:contractAddress, originationFee:originationFee, available:available, lenderSplit:lenderSplit, text:text, collateralAmount:NodeLender(lendingContractAddress).nodeCollateralAmount(), lastPaid:NodeLender(lendingContractAddress).lastPaid(), lastReward:NodeLender(lendingContractAddress).lastRewardAmount(), deploymentBlock:NodeLender(lendingContractAddress).borrowerDeploymentBlock()});
            ret[i] = newContract;
        }
        return ret;
    }
